...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Procesar creación
    $result = $contractsController->store($formData);
...
$createData = $contractsController->create();
if (isset($createData['success']) && !$createData['success']) {
    $error_message = $createData['message'];
    $awardees = [];
    $fiscal_years = [];
    $zones = [];
    $external_categories = [];
    ...
    $awardees = $createData['awardees'] ?? [];
    $fiscal_years = $createData['fiscal_years'] ?? [];
    $zones = $createData['zones'] ?? [];
    $external_categories = $createData['external_categories'] ?? [];
    $internal_categories = $createData['internal_categories'] ?? [];
    $page_title = $createData['page_title'] ?? 'Crear Nuevo Contrato';
}
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title">
                            ...
                            <?php echo htmlspecialchars($page_title ?? 'Crear Nuevo Contrato'); ?>
                        </h5>
                                ...
                                <div class="col-12">
                                    <div class="d-flex justify-content-end gap-2 mt-4">
                                        ...
                                        <button type="submit" class="btn btn-primary">
                                            <i class="ri-save-line"></i> Guardar Contrato
                                        </button>
                                    </div>
                                </div>
                            </div>
...
async function loadStalls() {
    ...
    try {
        console.log('Cargando locales para zona:', zoneId, 'sector:', sectorId);
        ...
        if (result.success) {
            ...
            console.log('Locales encontrados:', result.stalls);
...
document.addEventListener('DOMContentLoaded', function() {
    // Aquí se pueden cargar datos existentes si fuera una edición
    <?php if (!empty($formData['business_categories'])): ?>
    selectedCategories = <?php echo json_encode($formData['business_categories']); ?>;
    renderCategories();
    updateCategoriesInput();
    <?php endif; ?>
    
    <?php if (!empty($formData['locations'])): ?>
    // Para locales se necesitaría más lógica para obtener los datos completos
    <?php endif; ?>
});
</script>
